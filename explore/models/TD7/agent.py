...
class TD7_Agent(object):
    def __init__(self, state_dim, action_dim, max_action, offline=False, hp=TD7_Config()):
        ...
        self.actor_optimizer = torch.optim.Adam(self.actor.parameters(), lr=hp.learning_rate)
        self.actor_target = copy.deepcopy(self.actor)

        self.critic = Critic(state_dim, action_dim, hp.zs_dim, hp.critic_hdim).to(self.device)
        self.critic_optimizer = torch.optim.Adam(self.critic.parameters(), lr=hp.learning_rate)
        self.critic_target = copy.deepcopy(self.critic)

        self.encoder = Encoder(state_dim, action_dim, hp.zs_dim, hp.enc_hdim).to(self.device)
        self.encoder_optimizer = torch.optim.Adam(self.encoder.parameters(), lr=hp.learning_rate)
    ...
    def load(self, filename, device="cpu"):
        models = torch.load(filename, weights_only=True, map_location=torch.device(device))
        self.actor.load_state_dict(models["actor"])
        self.critic.load_state_dict(models["critic"])
        self.fixed_encoder.load_state_dict(models["fixed_encoder"])

        self.actor_target.load_state_dict(self.actor.state_dict())
        self.critic_target.load_state_dict(self.critic.state_dict())
        self.fixed_encoder_target.load_state_dict(self.fixed_encoder.state_dict())
        self.encoder.load_state_dict(self.fixed_encoder.state_dict())
    ...
    def select_action(self, state, use_checkpoint=False, use_exploration=True):
        with torch.no_grad():
            ...
            return action.clamp(-1, 1).cpu().data.numpy() * self.max_action  # .flatten()

    def evaluate_critic(self, state):
        with torch.no_grad():
            state = torch.tensor(state, dtype=torch.float, device=self.device)
            zs = self.fixed_encoder.zs(state)
            action = self.actor(state, zs).clamp(-1, 1)
            zsa = self.fixed_encoder.zsa(zs, action)
            Q = self.critic(state, action, zsa, zs).mean(axis=1)
            
        return Q.cpu().data.numpy()

    def train(self, writer: Writer, clone_buffer=None, multi=1):

        indices, state, action, next_state, reward, not_done = self.replay_buffer.sample(multi)

        if multi == 1:
            self.train_step(indices, state, action, next_state, reward, not_done, writer, clone_buffer)
        else:
            indices = indices.reshape(multi, self.replay_buffer.batch_size)
            state = state.reshape(multi, self.replay_buffer.batch_size, -1)
            action = action.reshape(multi, self.replay_buffer.batch_size, -1)
            next_state = next_state.reshape(multi, self.replay_buffer.batch_size, -1)
            reward = reward.reshape(multi, self.replay_buffer.batch_size, -1)
            not_done = not_done.reshape(multi, self.replay_buffer.batch_size, -1)

            for i in range(multi):
                self.train_step(
                    indices[i], state[i], action[i], next_state[i], reward[i], not_done[i], writer, clone_buffer
                )

    def train_step(self, indices, state, action, next_state, reward, not_done, writer, clone_buffer):
        ...
        writer.add("loss_enc", encoder_loss.item())
        ...
        writer.add("loss_Q", critic_loss.item())
        ...
        self.replay_buffer.update_priority(priority, indices)
        ...
        if self.training_steps % self.hp.policy_freq == 0:
            ...
            writer.add("value_pi", Q.mean().item())

            actor_loss = -Q.mean()
            if self.offline:
                actor_loss = actor_loss + self.hp.lmbda * Q.abs().mean().detach() * F.mse_loss(actor, action)

            if clone_buffer is not None:
                c_state, c_action = clone_buffer.sample(self.hp.batch_size)
                c_fixed_zs = self.fixed_encoder.zs(c_state)
                bc_loss = F.mse_loss(self.actor(c_state, c_fixed_zs), c_action)
                actor_loss = actor_loss + self.hp.lmbda * bc_loss

                writer.add("loss_pi_BC", bc_loss.item())
